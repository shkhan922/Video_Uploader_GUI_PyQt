...
class RightPanelContent(QWidget):
    def __init__(self, parent=None):
        super(RightPanelContent, self).__init__(parent)
        self.layout = QVBoxLayout(self)
        self.layout.setAlignment(Qt.AlignTop)

class VideoContent(RightPanelContent):
    def __init__(self, parent=None):
        super(VideoContent, self).__init__(parent)
        self.video_frame_1 = VideoFrame(self)
        self.video_frame_2 = VideoFrame(self)
        self.video_frames = [self.video_frame_1, self.video_frame_2]
        for video_frame in self.video_frames:
            self.layout.addWidget(video_frame)

class FormContent(RightPanelContent):
    def __init__(self, parent=None):
        super(FormContent, self).__init__(parent)
        self.label = QLabel("Form for Upload to Screen Goes Here", self)
...
class VehicleMonitoringApp(QMainWindow):
    def __init__(self):
        ...
        # Content widgets for the right panel
        self.video_content = VideoContent(self.right_panel)
        self.form_content = FormContent(self.right_panel)
...
# New buttons
        ...
        self.btn_setup.clicked.connect(self.preview)
...
# Add buttons to the Main tab
    ...
    def update_right_panel(self, button_text):
        ...
        if button_text == "Main":
            self.right_panel.layout().addWidget(self.video_content)
        elif button_text == "Upload to Screen":
            self.right_panel.layout().addWidget(self.form_content)
    ...
    def preview(self):
        ...
        self.stream_threads = [StreamThread('http://192.168.1.3:5000/video_feed') for _ in self.video_frames]
